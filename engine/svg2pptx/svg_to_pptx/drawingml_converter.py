...
def convert_g(elem: ET.Element, ctx: ConvertContext) -> ShapeResult | None:
    ...
    if rotate_pivot is not None and angle_deg:
        ...
        # 2026-07-02 saopan扫盘揪出（B2-14·vendor 代码最小修·与 drawingml_context.child 的
        # translate 漏乘 scale 同型病）：pivot 是元素局部坐标，换算到 slide 像素必须先乘祖先
        # scale 再加 translate（同 drawingml_utils.ctx_x 的不变量；convert_text 里的 rotate
        # pivot 就是这么算的）——旧写法漏乘 scale，祖先带 scale 时补偿量算错、整组旋转错位。
        pivot_ex = (cx_svg * ctx.scale_x + ctx.translate_x) * EMU_PER_PX
        pivot_ey = (cy_svg * ctx.scale_y + ctx.translate_y) * EMU_PER_PX
