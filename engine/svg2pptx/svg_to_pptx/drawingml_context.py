...
class ConvertContext:
    ...
    def child(
        ...
        return ConvertContext(
            ...
            # 2026-07-02 saopan扫盘揪出（B2-13·vendor 代码最小修）：子级 translate 此前没乘父级
            # scale——scalar 路径的不变量是 x_abs = x*scale_x + translate_x（见 drawingml_utils.ctx_x），
            # 复合 translate(t)∘scale(s)∘translate(d) 数学上 = scale s·offset t+s*d，旧写法 t+d 把
            # 嵌在 scale 组里的 <text> 等 scalar 路径元素整体错位（如 translate(10) scale(2) 内嵌
            # translate(100) 的 x=5 → 旧算 120，正确 220）。matrix 路径不受影响（矩阵乘法本来就对）。
