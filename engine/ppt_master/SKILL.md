...
| Anything else — bare template names ("用 academic_defense"), style descriptions ("麦肯锡风格"), brand mentions ("某品牌风格"), vague intent ("想用个模板"), or silence | Skip Step 3, free design |
...
> Bare names ("academic_defense", "某品牌", "anthropic") do NOT trigger Step 3 even if a matching directory exists in the library. The user must give a path. AI must not "helpfully" resolve a name to a path.
